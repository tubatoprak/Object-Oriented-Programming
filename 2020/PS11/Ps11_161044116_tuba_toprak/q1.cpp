...
#include <cstdlib>
#include <time.h>
//I had a hard time my homework, I learned and wrote how to do it on the internet, but I learned.
using namespace std;
...
void Doodlebug::move()
{
	...
	else if ((y<WORLDSIZE-1) && (world->getAt(x,y+1)!=NULL) && (world->getAt(x,y+1)->getType() == ANT))
	{
		delete (world->grid[x][y+1]);  
		world->grid[x][y+1] = this;    
		world->setAt(x,y,NULL);  starveTicks =0 ;
		y++;
		return;
	}
...
{ 
	...
	{
		int x = rand() % WORLDSIZE;int y = rand() % WORLDSIZE;
		if (w.getAt(x,y) == NULL)
		{
			antcount++;
